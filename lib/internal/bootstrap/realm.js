...
// passed by node::RunBootstrapping() // 编译时，会被视为被包裹在一个函数中，并且函数的参数通过 node::RunBootstrapping() 传递
/* global process, getLinkedBinding, getInternalBinding, primordials */
/* 在 Node.js 的环境中是预定义的一些全局对象和函数
process：Node.js 运行时的进程对象，包含有关当前 Node.js 进程的信息和控制方法。
getLinkedBinding： 获取与 Node.js 链接的第三方 C++ 库，扩充 Nodejs 的能力
getInternalBinding：获取 Node.js 内部 C++ 核心模块。使JavaScript 代码可以访问底层系统资源或执行底层操作。
primordials：Node.js 内部使用的一组基本函数和对象，用于实现核心功能。
 */
const {
  ArrayFrom,
